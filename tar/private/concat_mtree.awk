...
# acts as a pure normalizer: blank lines are dropped, `#` comments are passed through, and every
# directory entry is written as a "full" entry with a trailing slash (a slash-free directory line
# would otherwise "change directory" and nest the lines that follow it, see default.awk).
#
# Each remaining line is a self-contained entry, keyed on its path:
...
#     is preserved.
#
# Inputs are expected to be specs produced by tar.bzl (`mtree_spec`, `mtree_mutate`, or the
# `tar()` macro's inline lines): one entry per line with every keyword spelled out. Other mtree
# features (`/set`, `/unset`, `..`) are not interpreted.
