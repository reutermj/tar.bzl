...
_mutate_mtree_attrs = {
    ...
    "_preserve_symlinks_pipeline": attr.label(
        default = Label("@tar.bzl//tar/private:preserve_symlinks.awk"),
        allow_single_file = True,
    ),
    "srcs": attr.label_list(
        allow_files = True,
...
def _mtree_mutate_impl(ctx):
    ...
    # Merge script_args first so built-in assignments below win on conflict
    for key, value in ctx.attr.script_args.items():
    ...
    ctx.actions.write(wrapper, "".join(lines))
    script_to_run = wrapper
    ...
    ctx.actions.run_shell(
        command = "AWKPATH=\"{awk_path}\" {awk} $@ > {out_mtree}".format(
            awk_path = awk_path,
            awk = ctx.executable._awk.path,
...
Inputs are expected to be specs produced by `mtree_spec`, `mtree_mutate`, or the `tar()`
macro's inline lines: one entry per line. Every line is first normalized by the default
`mtree_mutate` pipeline, so blank lines are dropped, comments are kept, and every directory
entry is written as a full entry with a trailing slash, which keeps a bare directory line in
one input from nesting the lines that follow it.
""",
